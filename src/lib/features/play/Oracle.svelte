<script lang="ts">
  ...
  const btnCls = "px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition";
  const docBtnCls = "text-indigo-300 hover:text-indigo-200 hover:underline font-bold";
</script>
...
<div class="max-w-2xl mx-auto space-y-6 pb-16">
  ...
  <section class={cardCls}>
    <h2 class="text-sm font-bold text-indigo-400 mb-1">❓ 예 / 아니오</h2>
    <p class="text-[11px] text-slate-500 mb-4">질문을 떠올리고, 가능성을 고른 뒤 물어보세요. "그리고/하지만"은 답에 변수를 더합니다.</p>
    ...
    {#if ynResult}
      <div class="mt-4 rounded-xl border border-slate-700 bg-slate-950/60 p-4">
        <p class="text-2xl font-bold text-slate-100">{ynResult.text}</p>
        <p class="text-[11px] text-slate-500 mt-1">d100 = {ynResult.roll}</p>
        {#if ynResult.twist}
  ...
  <!-- 2. 씬 뽑기 -->
  <section class={cardCls}>
    <h2 class="text-sm font-bold text-indigo-400 mb-1">🎬 씬 뽑기</h2>
    <p class="text-[11px] text-slate-500 mb-4">내 세계관의 장소와 인물을 무작위로 엮어 장면을 만듭니다.</p>
    ...
    {#if scene}
      <div class="mt-4 rounded-xl border border-slate-700 bg-slate-950/60 p-4 text-sm leading-relaxed text-slate-200">
        ...
        {#if !scene.location || !scene.character}
          <p class="text-[11px] text-slate-500 mt-2">※ 장소/인물 문서가 더 쌓이면 조합이 풍부해져요.</p>
        {/if}
      </div>
    {/if}
  </section>
  ...
  <!-- 3. 전개 뽑기 -->
  <section class={cardCls}>
    <h2 class="text-sm font-bold text-indigo-400 mb-1">🌀 전개 뽑기</h2>
    <p class="text-[11px] text-slate-500 mb-4">이야기가 막혔을 때 — 사건의 방향, 관련 설정, 해석용 단서 두 개를 뽑습니다.</p>

    <button class={btnCls} on:click={rollEvent}>전개 굴리기</button>

    {#if event}
      <div class="mt-4 rounded-xl border border-slate-700 bg-slate-950/60 p-4 space-y-2">
        <p class="text-lg font-bold text-slate-100">{event.focus}</p>
        ...
        <p class="text-[11px] text-slate-500 pt-1">세 조각을 엮어 해석하는 건 당신의 몫 — 그게 오라클의 규칙이에요.</p>
      </div>
    {/if}
  </section>
  ...
  <section class={cardCls}>
    <div class="flex items-center justify-between">
      <h2 class="text-sm font-bold text-indigo-400">⚙️ 단어 테이블</h2>
      {#if !editing}
        <button class="text-xs text-indigo-300 hover:underline" on:click={openEditor}>편집</button>
      {/if}
    </div>

    {#if editing}
      <p class="text-[11px] text-slate-500 mt-1 mb-3">한 줄에 한 단어씩. 비워두면 해당 칸은 기본값으로 돌아갑니다.</p>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <label class="block text-[11px] font-bold text-slate-400 mb-1">씬 분위기</label>
          <textarea bind:value={editText.moods} rows="8" class="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500"></textarea>
        </div>
        <div>
          <label class="block text-[11px] font-bold text-slate-400 mb-1">사건의 방향</label>
          <textarea bind:value={editText.eventFocus} rows="8" class="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500"></textarea>
        </div>
        <div>
          <label class="block text-[11px] font-bold text-slate-400 mb-1">단서 — 동사</label>
          <textarea bind:value={editText.sparkVerbs} rows="8" class="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500"></textarea>
        </div>
        <div>
          <label class="block text-[11px] font-bold text-slate-400 mb-1">단서 — 명사</label>
          <textarea bind:value={editText.sparkNouns} rows="8" class="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 outline-none focus:border-indigo-500"></textarea>
        </div>
      </div>
      <div class="flex gap-2 mt-3">
        <button class={btnCls} on:click={saveTables}>저장</button>
        <button class="px-4 py-2.5 rounded-xl border border-slate-700 text-slate-400 text-sm hover:border-slate-500 transition" on:click={() => (editing = false)}>취소</button>
