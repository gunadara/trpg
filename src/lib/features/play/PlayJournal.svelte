<script lang="ts">
  ...
  function cardStyle(log: SessionLog): { icon: string; accent: string; isNarrative: boolean } {
    switch (log.type) {
      ...
      default:           return { icon: '◆', accent: 'border-l-slate-600', isNarrative: false };
    }
  }
...
{:else}
  <div class="flex flex-col h-full">
    ...
    <div class="flex-1 overflow-y-auto px-1 py-2 space-y-2.5">
      {#if logs.length === 0}
        <p class="text-center text-slate-600 text-sm py-12">
      ...
      {#each logs as log (log.id)}
        ...
        {#if s.isNarrative}
          <!-- 내가 쓴 서사 -->
          <div class="group relative rounded-xl bg-slate-800/40 px-4 py-3">
            <p class="text-[15px] leading-relaxed text-slate-100 whitespace-pre-wrap">{log.content}</p>
            <div class="flex items-center gap-2 mt-1.5">
              <span class="text-[10px] text-slate-600">{timeOf(log.timestamp)}</span>
              <button on:click={() => deleteLog(log.id)} class="text-[10px] text-slate-700 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition">삭제</button>
            </div>
          </div>
        {:else}
          <!-- 판정 카드 -->
          <div class="group relative flex items-start gap-2.5 rounded-lg bg-slate-900/60 border-l-2 {s.accent} pl-3 pr-4 py-2">
            <span class="text-sm mt-0.5">{s.icon}</span>
            <div class="flex-1 min-w-0">
              <p class="text-[13px] text-slate-300 leading-snug">{log.content}</p>
    ...
    <!-- 입력 영역 -->
    <div class="border-t border-slate-800 pt-3 mt-1 space-y-2.5">

      <!-- 예/아니오 펼침 -->
      {#if showAsk}
        <div class="rounded-xl border border-slate-700 bg-slate-900/80 p-3 space-y-2.5">
          <input
            type="text"
            bind:value={question}
      ...
      <div class="flex items-end gap-2">
        <textarea
          ...
          class="flex-1 resize-none rounded-xl border border-slate-700 bg-slate-900 px-3.5 py-2.5 text-[15px] text-slate-100 outline-none focus:border-indigo-500 leading-relaxed"
        ></textarea>
        <button
          on:click={writeNarrative}
          class="shrink-0 px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-white text-slate-900 text-sm font-bold transition"
        >기록</button>
      </div>

      <!-- 판정 버튼 줄 -->
      <div class="flex flex-wrap items-center gap-1.5">
        <button on:click={() => (showAsk = !showAsk)} class="px-3 py-1.5 rounded-lg text-xs border transition {showAsk ? 'border-indigo-500 text-indigo-300 bg-indigo-500/10' : 'border-slate-700 text-slate-300 hover:border-indigo-500'}">❓ 예/아니오</button>
        <button on:click={doScene} class="px-3 py-1.5 rounded-lg text-xs border border-slate-700 text-slate-300 hover:border-emerald-500 transition">🎬 씬</button>
        <button on:click={doEvent} class="px-3 py-1.5 rounded-lg text-xs border border-slate-700 text-slate-300 hover:border-violet-500 transition">🌀 전개</button>
        <span class="mx-1 text-slate-700">|</span>
        {#each DICE as sides}
          <button on:click={() => doDice(sides)} class="px-2.5 py-1.5 rounded-lg text-[11px] border border-slate-800 text-slate-400 hover:border-sky-500 hover:text-sky-300 transition">d{sides}</button>
        {/each}
      </div>
    </div>
  </div>
