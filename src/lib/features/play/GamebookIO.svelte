...
<div class="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50"
  ...
  <div class="w-full max-w-2xl max-h-[88vh] flex flex-col rounded-2xl border border-slate-700 bg-slate-900"
    ...
    <div class="flex items-center justify-between p-4 border-b border-slate-800 shrink-0">
      ...
      <button on:click={onClose} class="text-slate-500 hover:text-slate-300">✕</button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
      ...
      {#if mode === 'import'}
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-950/50">
          <button on:click={() => (showGuide = !showGuide)} class="w-full flex items-center justify-between px-3 py-2.5 text-sm text-slate-300">
            <span>📖 작성법 {showGuide ? '접기' : '보기'}</span>
            <span class="text-slate-600">{showGuide ? '▲' : '▼'}</span>
          </button>
          {#if showGuide}
            <div class="px-3 pb-3 space-y-3">
              <pre class="text-[11px] text-slate-400 whitespace-pre-wrap leading-relaxed font-mono bg-slate-950 rounded-lg p-3 border border-slate-800">{FORMAT_GUIDE}</pre>
              <div class="flex flex-wrap gap-2">
                <button on:click={loadExample} class="px-3 py-1.5 rounded-lg text-xs border border-slate-700 text-slate-300 hover:border-indigo-500 transition">예시 불러오기</button>
        ...
        <textarea
          ...
          rows="12"
          placeholder={'# 첫 장면\n여기에 본문을 쓰고…\n\n- 선택지 -> 다음 장면'}
          class="w-full rounded-xl border border-slate-700 bg-slate-950 px-3.5 py-3 text-sm text-slate-100 leading-relaxed outline-none focus:border-indigo-500 resize-y font-mono"
        ></textarea>
      ...
      {:else}
        <!-- 내보내기 -->
        <p class="text-[11px] text-slate-500">이 텍스트를 복사해서 보관하거나, 다른 곳에서 「가져오기」로 다시 불러올 수 있어요.</p>
        <textarea
          value={text}
          readonly
          rows="14"
          class="w-full rounded-xl border border-slate-700 bg-slate-950 px-3.5 py-3 text-sm text-slate-300 leading-relaxed resize-y font-mono"
        ></textarea>
      {/if}
    </div>

    <!-- 푸터 -->
    <div class="flex items-center justify-between p-4 border-t border-slate-800 shrink-0">
      <span class="text-[11px] text-emerald-400">{copied ? `✓ ${copied} 복사됨` : ''}</span>
      <div class="flex items-center gap-2">
        {#if mode === 'export'}
          <button on:click={() => copy('전체', text)} class="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition">전체 복사</button>
        {:else}
          <button on:click={onClose} class="px-3 py-2 text-sm text-slate-400 hover:text-slate-200">취소</button>
          <button on:click={doImport} class="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition">
            {book ? '교체' : '가져오기'}
          </button>
        {/if}
      </div>
