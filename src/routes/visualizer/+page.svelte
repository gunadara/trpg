...
<div class="h-screen flex flex-col bg-canvas overflow-hidden">
  <!-- 헤더 -->
  <header class="p-4 bg-surface border-b border-line flex items-center justify-between">
    <div class="flex items-center gap-3">
      <button on:click={() => history.back()} class="text-muted hover:text-ink transition text-sm flex items-center gap-1">
        ← 뒤로
      </button>
      <h1 class="text-lg font-bold text-ink flex items-center gap-2">
        🕸️ 세계관 관계도 <span class="text-xs font-normal text-muted">(Visualization)</span>
      </h1>
    </div>

    <div class="flex items-center gap-2">
      <span class="text-xs text-muted">{filteredDocs.length} / {allDocs.length} 노드</span>
    </div>
  </header>

  <!-- 카테고리 필터 칩 -->
  <div class="px-4 py-2.5 bg-surface/60 border-b border-line flex items-center gap-1.5 overflow-x-auto whitespace-nowrap [scrollbar-width:thin]">
      ...
      <div class="absolute inset-0 bg-black/40 flex items-end md:items-center justify-center p-4 z-20"
        ...
        <div class="w-full max-w-sm rounded-2xl border border-line bg-surface p-5 space-y-4"
          ...
          <div class="flex justify-between pt-1">
            ...
            <button class="text-xs text-muted hover:text-muted" on:click={() => (edgeSel = null)}>닫기</button>
          </div>
        </div>
      </div>
    {/if}
