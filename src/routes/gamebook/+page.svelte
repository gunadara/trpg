...
<div class="h-screen flex flex-col bg-canvas text-ink">
  <header class="shrink-0 p-4 bg-surface border-b border-line flex items-center justify-between gap-3">
    ...
    {#if book}
      <div class="flex items-center gap-2">
        <button on:click={openImportOver} class="text-xs text-muted hover:text-primary transition">📥 텍스트로 채우기</button>
        <button on:click={openExport} class="text-xs text-muted hover:text-primary transition">📤 내보내기</button>
        <a href="/gamebook/play?book={book.id}" class="px-3 py-1.5 rounded-lg bg-primary hover:opacity-90 text-white text-xs font-bold transition">▶ 플레이</a>
      </div>
    {/if}
  </header>
  ...
  <div class="flex-1 flex overflow-hidden">
    <!-- 좌: 게임북 + 장면 목록 -->
    <aside class="w-full md:w-60 shrink-0 border-r border-line bg-surface/40 overflow-y-auto p-3 space-y-4 {sceneId ? 'hidden md:block' : 'block'}">
      <!-- 게임북 선택 -->
      <div>
        <div class="flex items-center justify-between mb-1.5">
          <span class="text-[10px] font-bold text-muted uppercase tracking-wider">게임북</span>
          <div class="flex items-center gap-2">
            <button on:click={openImportNew} class="text-[11px] text-emerald-400 hover:underline">📥 텍스트</button>
            <button on:click={newBook} class="text-[11px] text-primary hover:underline">+ 새로</button>
          </div>
        </div>
          ...
          <div class="flex items-center gap-1 mb-1">
            <button
              ...
              class="flex-1 text-left px-2.5 py-1.5 rounded-lg text-xs transition truncate
                     {bookId === b.id ? 'bg-primary/20 border border-primary text-primary' : 'border border-transparent text-muted hover:bg-bubble'}"
            >📓 {b.title}</button>
            <button on:click={() => removeBook(b)} class="text-subtle hover:text-rose-400 text-xs px-1">×</button>
          </div>
        {/each}
        {#if books.length === 0}
          <p class="text-[11px] text-subtle px-1">+ 새로 눌러 시작</p>
        {/if}
      </div>
      ...
      {#if book}
        <div class="border-t border-line pt-3">
          <div class="flex items-center justify-between mb-1.5">
            <span class="text-[10px] font-bold text-muted uppercase tracking-wider">장면 ({book.scenes.length})</span>
            <button on:click={newScene} class="text-[11px] text-primary hover:underline">+ 장면</button>
          </div>
          {#each book.scenes as s (s.id)}
            <button
              ...
              class="w-full text-left px-2.5 py-1.5 rounded-lg text-xs transition mb-1 flex items-center gap-1.5
                     {sceneId === s.id ? 'bg-bubble text-ink' : 'text-muted hover:bg-bubble'}"
            >
    ...
    <!-- 우: 장면 에디터 -->
    <main class="flex-1 overflow-y-auto p-4 md:p-6 {sceneId ? 'block' : 'hidden md:block'}">
      {#if !book}
        <div class="h-full flex items-center justify-center text-muted text-sm">왼쪽에서 게임북을 만들어 보세요.</div>
      {:else if !scene}
        <div class="h-full flex items-center justify-center text-muted text-sm">「+ 장면」으로 첫 장면을 추가하세요.</div>
      {:else}
        <div class="max-w-2xl mx-auto space-y-5">
          <!-- 폰 전용: 목록으로 -->
          <button on:click={() => (sceneId = null)} class="md:hidden text-xs text-muted hover:text-primary transition flex items-center gap-1">← 목록</button>
          <!-- 장면 헤더 -->
          <div class="flex items-center justify-between gap-3">
            <input
              ...
              placeholder="장면 제목"
              class="flex-1 bg-transparent text-xl font-bold text-ink outline-none border-b border-transparent focus:border-line pb-1"
            />
            <div class="flex items-center gap-2 shrink-0">
              {#if book.startSceneId !== scene.id}
                <button on:click={() => gamebooks.setStart(book.id, scene.id)} class="text-[11px] text-muted hover:text-emerald-400">시작 장면으로</button>
              {:else}
          ...
          <!-- 본문 -->
          <div>
            <label class="block text-xs font-bold text-muted mb-1.5">본문 (플레이어가 읽는 묘사)</label>
            {#key scene.id}
              <MentionTextarea
                value={scene.body}
                rows={6}
          ...
          {#if scene.linkedDocs && scene.linkedDocs.length > 0}
            <div class="rounded-xl border border-line bg-surface/40 p-3">
              <p class="text-[11px] font-bold text-muted mb-2">🔗 이 장면에 등장 (본문 @멘션 자동 연결)</p>
              <div class="flex flex-wrap gap-1.5">
                {#each scene.linkedDocs as id (id)}
                  <span class="inline-flex items-center gap-1.5 rounded-full border border-line bg-canvas px-2.5 py-1 text-xs text-muted">
                    {docTitle(id)}
                    <button on:click={() => pullDocText(id)} class="text-[10px] text-primary hover:text-primary" title="이 문서의 설정을 본문에 가져오기">⬇설정</button>
                  </span>
                {/each}
              </div>
            </div>
          ...
          <div class="rounded-xl border border-dashed border-rose-900/60 bg-rose-950/10 p-3">
            ...
            <textarea
              ...
              class="w-full rounded-lg border border-line bg-canvas px-3 py-2 text-sm text-muted outline-none focus:border-rose-700 resize-y"
            ></textarea>
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-xs font-bold text-muted">선택지</label>
              <button on:click={() => gamebooks.addChoice(book.id, scene.id)} class="text-xs text-primary hover:underline">+ 선택지 추가</button>
            </div>

            {#if scene.choices.length === 0}
              <p class="text-[11px] text-subtle rounded-lg border border-dashed border-line px-3 py-3 text-center">
            ...
            <div class="space-y-2">
              {#each scene.choices as c (c.id)}
                <div class="flex items-center gap-2 rounded-xl border border-line bg-surface/60 p-2.5">
                  <span class="text-subtle text-sm shrink-0">▸</span>
                  <input
                    type="text"
                    value={c.text}
                    on:input={(e) => gamebooks.updateChoice(book.id, scene.id, c.id, { text: e.currentTarget.value })}
                    placeholder="선택지 문구 (예: 문을 연다)"
                    class="flex-1 min-w-0 bg-transparent text-sm text-ink outline-none"
                  />
                  ...
                  <select
                    ...
                    class="shrink-0 max-w-[140px] rounded-lg border border-line bg-canvas px-2 py-1.5 text-xs text-ink outline-none focus:border-primary"
                  >
                  ...
                  <button on:click={() => gamebooks.removeChoice(book.id, scene.id, c.id)} class="text-subtle hover:text-rose-400 text-xs shrink-0 px-1">×</button>
                </div>
              {/each}
            </div>
