<script lang="ts">
  ...
  function createNew() {
    ...
    refreshList();
    goto(`/world/events/${newDoc.id}`);
  }

  function openDoc(doc: WorldDoc) {
    goto(`/world/events/${doc.id}`);
  }
...
<DocListLayout
  icon={META.icon}
  title={META.label}
  subtitle="전쟁, 재난, 쿠데타, 의문사 등 세계관에 영향을 주는 사건들을 정리합니다."
  primaryText="새 사건 문서"
  onPrimary={createNew}
          ...
          >
            ...
            >
              ...
              >
                ...
                {:else}
                  <span class="truncate">{(doc.title ?? '인물').slice(0, 2)}</span>
                {/if}
              </div>
