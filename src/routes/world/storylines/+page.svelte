...
<script lang="ts">
  ...
  function createNew() {
    ...
    refreshList();
    goto(`/world/storylines/${newDoc.id}`);
  }
...
<DocListLayout
  icon={META.icon}
  title={META.label}
  subtitle="메인/서브 퀘스트, 시즌 아크, 에피소드 흐름 등을 정리합니다."
  primaryText="새 스토리 라인"
  onPrimary={createNew}
  ...
  {#if docs.length === 0}
    <div class="h-full flex items-center justify-center px-4 py-6">
      <p class="text-xs text-slate-500 dark:text-slate-400 text-center leading-relaxed max-w-[260px]">
        아직 등록된 스토리 문서가 없습니다.<br />
        우측 상단의 <strong>“새 스토리 라인”</strong> 버튼을 눌러 시작해보세요.
      </p>
    </div>
