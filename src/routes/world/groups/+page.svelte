<script lang="ts">
  ...
  const CATEGORY: CategoryId = 'groups';
...
$: filteredDocs =
  ...
  // ✅ URL ?id=에서 값 받아온 적 있는지 체크하는 플래그
  ...
  // ✅ URL ?id= → 처음 들어올 때 한 번만 selectedId로 반영
  $: if (!initializedFromParam) {
    ...
    if (paramId) {
      ...
      if (target && target.category === CATEGORY) {
        // 쿼리로 넘어온 id가 유효한 groups 문서면 그걸 선택
        selectedId = target.id;
      } else if (!selectedId && docs[0]) {
        // 못 찾으면 첫 번째 문서로 fallback
        selectedId = docs[0].id;
      }
    } else if (!selectedId && docs[0]) {
      // 쿼리 없고 selectedId도 없으면 첫 번째 문서 선택
      selectedId = docs[0].id;
    }
...
<section class="h-full flex flex-col">
  <header class="mb-4 flex items-center justify-between gap-3">
    ...
    <p class="text-sm text-slate-500 dark:text-slate-400">
        길드, 종단, 학회, 비밀 조직 등 세계관의 각종 단체를 관리합니다.
      </p>
    </div>
  ...
  <div class="flex-1 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(260px,320px)_minmax(0,1fr)]">
    <aside
  ...
  <div class="flex-1 flex items-center justify-center px-4 py-6">
    ...
    >
      아직 등록된 단체 문서가 없습니다.
      <br />
      우측 상단의 <strong>“새 단체 문서”</strong> 버튼을 눌러 시작해보세요.
    </p>
  </div>
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              placeholder="예: 달빛 아래서 계약을 맺는 비밀 길드"
            />
          </div>
        </div>
        ...
        <div class="flex-1 flex flex-col gap-2">
          ...
          <div class="relative flex-1">
            <textarea
              ...
              placeholder="이 단체의 목적, 조직 구조, 활동 방식, 주요 인물 등을 적어보세요."
            ></textarea>
    ...
    <div class="flex flex-col items-end gap-1 pt-2 border-t border-slate-100 dark:border-slate-800">
      ...
      {:else}
        <div class="m-auto text-center text-sm text-slate-500 dark:text-slate-400 space-y-2">
          <p>왼쪽에서 단체 문서를 선택하거나,</p>
          <p>
            우측 상단의 <strong>“새 단체 문서”</strong> 버튼을 눌러 첫 문서를 만들어보세요.
          </p>
        </div>
      {/if}
    </section>
